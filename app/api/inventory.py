...
async def read_all_requisitions(
    start_time: datetime = None,
    end_time: datetime = None,
    db: AsyncSession = Depends(get_db)):
    """获取所有领料申请，支持按照时间窗口筛选"""
    stmt = select(RequisitionModel)
    if start_time:
        stmt = stmt.where(RequisitionModel.apply_date >= start_time)
    if end_time:
        stmt = stmt.where(RequisitionModel.apply_date <= end_time)
...
async def update_requisition(
    ...
    # 耗材审批流转：检查库存是否充足
    from app.models.models import RequisitionStatusEnum
    if new_status and new_status == RequisitionStatusEnum.Approved:
        # 当审批流转变为“Approved”时
        cons_result = await db.execute(select(ConsumableModel).where(ConsumableModel.consumable_id == db_requisition.consumable_id))
        db_consumable = cons_result.scalar_one_or_none()
        if not db_consumable:
            raise HTTPException(status_code=404, detail="关联耗材不存在")
        if db_consumable.quantity < db_requisition.quantity:
            raise HTTPException(status_code=400, detail=f"审批失败：库存不足 (当前库存: {db_consumable.quantity}, 申请数量: {db_requisition.quantity})")
